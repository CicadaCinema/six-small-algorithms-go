...
func place_queens_4(board [][]string, num_rows, c int) bool {
	...
	// now we know c < num_rows

	if !board_is_legal(board, num_rows) {
		// if this board is an illegal state, we know that placing more queens won't get us anywhere
		return false
	}

	// we need to assign a queen to column c

	for r := 0; r < num_rows; r++ {
		// try placing a queen at (r,c)
		board[r][c] = "Q"

		// if this results in a valid solution, great! return true
		if place_queens_4(board, num_rows, c+1) {
			return true
		}

		// if we have reached this point, we know that placing a queen here cannot result in a valid solution.
		// so we undo and move to the next iteration of the loop
		board[r][c] = "."
	}

	// now we know that no assignment of queens in column c can result in a valid solution.
	// but there must be a queen in this column, so there is no solution with the current configuration
	return false
}
